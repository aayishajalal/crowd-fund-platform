...
const CampaignDetailsPage = ({ campaign }) => {
  return (
    <div className="min-h-screen py-10 bg-gray-100">
      <div className="max-w-4xl p-8 mx-auto bg-white rounded-lg shadow-lg">
        ...
        <div className="mt-6">
          <h3 className="text-xl font-semibold">Goal: ${campaign.goalAmount}</h3>
          <div className="flex items-center mt-4">
            ...
            <span className="ml-4">{Math.round((campaign.raisedAmount / campaign.goalAmount) * 100)}% funded</span>
          </div>
        </div>
        ...
        <div className="mt-6">
          ...
          <ul>
            {campaign.milestones.map((milestone, index) => (
              <li key={index}>
                {milestone.name}: ${milestone.amount} - {Math.round((milestone.raised / milestone.amount) * 100)}% funded
              </li>
            ))}
          </ul>
        </div>
